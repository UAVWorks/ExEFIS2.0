...
bool SKFilter::update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
{
 ...
 if((gx != gx_) || (gy != gy_) || (gz != gz_)) {
	...
	if (gyroUpdated_) {	
		...
		integral[0] = 2* ( (gyroPrev[0] * _dt) + ((gx_ - gyroPrev[0]) * 0.5f * _dt));
		integral[1] = 2* ( (gyroPrev[1] * _dt) + ((gy_ - gyroPrev[1]) * 0.5f * _dt));
		integral[2] = 2* ( (gyroPrev[2] * _dt) + ((gz_ - gyroPrev[2]) * 0.5f * _dt));
		
		_Euler[0] += integral[0];
		_Euler[1] += integral[1];
		magHeading += integral[2];
		//_Euler[2] += integral[2];
	}
	...
	if (accelUpdated_)
	{
		...
		{
			if (_Euler[0] > 0) _Euler[0] -= 0.1f * _dt; //move at 0.1 rad / s
			else _Euler[0] += 0.1f * _dt;
			if (_Euler[1] > 0) _Euler[1] -= 0.1f * _dt;   //move at 0.1 rad / s
			else _Euler[1] += _dt;
		}
	}
